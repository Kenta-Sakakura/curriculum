...
-- [回答]
select store_name AS（つけなくても可） 店舗名 from store_table
...
-- [回答]
select avg(price_nameabc)from goods_table
-- 5. 店舗コード='EA01'の在庫数の平均値より大きい在庫数を持つ店舗コードを抽出しなさい。
-- [回答]
select store_code, quantity from stock_code where quantity > (select avg(quantity) from stock_table)
-- 6. 商品テーブルに「商品コード='M001'、商品名='マフラー'、単価=4500円、更新日付=本日日付」のデータを追加しなさい。※実行後のSELECT結果も貼付すること。
-- [回答]
insert into goods_table(
goods_code, goods_name, price_nameabc, update_day)
values
('M001', 'マフラー', 4500円, '2020/7/31');
